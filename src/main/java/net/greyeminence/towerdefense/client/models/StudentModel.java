...
import net.greyeminence.towerdefense.IArmPoseTransformerCustom;
...
public class StudentModel<T extends LivingEntity> extends EntityModel<T> implements ArmedModel {
    ...
    public StudentModel(ModelPart modelPart, boolean isSlim) {
    ...
    public static MeshDefinition createMesh(CubeDeformation cubeDeformation, boolean bool) {
        ...
        partdefinition.addOrReplaceChild("head", CubeListBuilder.create().texOffs(0, 0).addBox(-4.0F, -8.0F, -4.0F, 8.0F, 8.0F, 8.0F, cubeDeformation), PartPose.offset(0.0F, 0.0F + 0.0F, 0.0F));
        partdefinition.addOrReplaceChild("hat", CubeListBuilder.create().texOffs(32, 0).addBox(-4.0F, -8.0F, -4.0F, 8.0F, 8.0F, 8.0F, cubeDeformation.extend(0.5F)), PartPose.offset(0.0F, 0.0F + 0.0F, 0.0F));
        partdefinition.addOrReplaceChild("body", CubeListBuilder.create().texOffs(16, 16).addBox(-4.0F, 0.0F, -2.0F, 8.0F, 12.0F, 4.0F, cubeDeformation), PartPose.offset(0.0F, 0.0F + 0.0F, 0.0F));
        partdefinition.addOrReplaceChild("right_arm", CubeListBuilder.create().texOffs(40, 16).addBox(-3.0F, -2.0F, -2.0F, 4.0F, 12.0F, 4.0F, cubeDeformation), PartPose.offset(-5.0F, 2.0F + 0.0F, 0.0F));
        partdefinition.addOrReplaceChild("left_arm", CubeListBuilder.create().texOffs(40, 16).mirror().addBox(-1.0F, -2.0F, -2.0F, 4.0F, 12.0F, 4.0F, cubeDeformation), PartPose.offset(5.0F, 2.0F + 0.0F, 0.0F));
        partdefinition.addOrReplaceChild("right_leg", CubeListBuilder.create().texOffs(0, 16).addBox(-2.0F, 0.0F, -2.0F, 4.0F, 12.0F, 4.0F, cubeDeformation), PartPose.offset(-1.9F, 12.0F + 0.0F, 0.0F));
        partdefinition.addOrReplaceChild("left_leg", CubeListBuilder.create().texOffs(0, 16).mirror().addBox(-2.0F, 0.0F, -2.0F, 4.0F, 12.0F, 4.0F, cubeDeformation), PartPose.offset(1.9F, 12.0F + 0.0F, 0.0F));
    ...
    public void prepareMobModel(T student, float p_102862_, float p_102863_, float p_102864_) {
        this.swimAmount = student.getSwimAmount(p_102864_);
    }
    ...
    public void setupAnim(T student, float float1, float float2, float float3, float float4, float float5) {
        boolean flag = student.getFallFlyingTicks() > 4;
        boolean flag1 = student.isVisuallySwimming();
        ...
        float f = 1.0F;
        if (flag) {
            f = (float)student.getDeltaMovement().lengthSqr();
            f /= 0.2F;
            f *= f * f;
        }
        ...
        boolean flag2 = student.getMainArm() == HumanoidArm.RIGHT;
        boolean flag3;
        if (student.isUsingItem()) {
            flag3 = student.getUsedItemHand() == InteractionHand.MAIN_HAND;
            if (flag3 == flag2) {
                this.poseRightArm(student);
            } else {
                this.poseLeftArm(student);
            }
        } else {
        ...
        this.setupAttackAnimation(student, float3);
        if (this.crouching) {
            this.body.xRot = 0.5F;
            var10000 = this.rightArm;
            var10000.xRot += 0.4F;
        ...
        if (this.swimAmount > 0.0F) {
            ...
            HumanoidArm humanoidarm = this.getAttackArm(student);
        ...
        if (student.getItemBySlot(EquipmentSlot.CHEST).isEmpty()) {
            if (student.isCrouching()) {
                this.cloak.z = 1.4F;
                this.cloak.y = 1.85F;
            } else {
                this.cloak.z = 0.0F;
    ...
    private void poseRightArm(T student) {
        switch (this.rightArmPose) {
            ...
            case CROSSBOW_CHARGE:
                AnimationUtils.animateCrossbowCharge(this.rightArm, this.leftArm, student, true);
                break;
            case CROSSBOW_HOLD:
                AnimationUtils.animateCrossbowHold(this.rightArm, this.leftArm, this.head, true);
                break;
            case SPYGLASS:
                this.rightArm.xRot = Mth.clamp(this.head.xRot - 1.9198622F - (student.isCrouching() ? 0.2617994F : 0.0F), -2.4F, 3.3F);
            ...
            default:
                this.rightArmPose.applyTransform(this, student, HumanoidArm.RIGHT);
        }

    }

    private void poseLeftArm(T student) {
        switch (this.leftArmPose) {
            ...
            case BOW_AND_ARROW:
                ...
                break;
            case CROSSBOW_CHARGE:
                AnimationUtils.animateCrossbowCharge(this.rightArm, this.leftArm, student, false);
                break;
            case CROSSBOW_HOLD:
                AnimationUtils.animateCrossbowHold(this.rightArm, this.leftArm, this.head, false);
                break;
            case SPYGLASS:
                this.leftArm.xRot = Mth.clamp(this.head.xRot - 1.9198622F - (student.isCrouching() ? 0.2617994F : 0.0F), -2.4F, 3.3F);
            ...
            default:
                this.leftArmPose.applyTransform(this, student, HumanoidArm.LEFT);
        }
    ...
    protected void setupAttackAnimation(T student, float float1) {
    ...
    public void copyPropertiesTo(StudentModel<T> studentModel) {
        super.copyPropertiesTo(studentModel);
        studentModel.leftArmPose = this.leftArmPose;
        studentModel.rightArmPose = this.rightArmPose;
        studentModel.crouching = this.crouching;
        studentModel.head.copyFrom(this.head);
        studentModel.hat.copyFrom(this.hat);
        studentModel.body.copyFrom(this.body);
        studentModel.rightArm.copyFrom(this.rightArm);
        studentModel.leftArm.copyFrom(this.leftArm);
        studentModel.rightLeg.copyFrom(this.rightLeg);
        studentModel.leftLeg.copyFrom(this.leftLeg);
    }
    ...
    private HumanoidArm getAttackArm(T student) {
        HumanoidArm humanoidarm = student.getMainArm();
        return student.swingingArm == InteractionHand.MAIN_HAND ? humanoidarm : humanoidarm.getOpposite();
    }
    ...
    public static enum ArmPose implements IExtensibleEnum {
        ...
        public <T extends LivingEntity> void applyTransform(StudentModel<T> model, T student, HumanoidArm arm) {
            if (this.forgeArmPose != null) {
                this.forgeArmPose.applyTransform(model, student, arm);
            }
